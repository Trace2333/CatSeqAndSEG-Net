...
class SelfDotMultiHeadAttention(nn.Module):
    ...
    def __init__(self, input_size, k_size, q_size, v_size, h, drop_rate):
        """多头的每一个头保持相同的大小
        NOTE：
        ...
        self.q_size = q_size
        for i in range(h):
            self.ks.append(nn.Linear(input_size, int(k_size / h), bias=False).to(device))
            self.vs.append(nn.Linear(input_size, int(v_size / h), bias=False).to(device))
            self.qs.append(nn.Linear(input_size, int(q_size / h), bias=False).to(device))
        self.out_fn = nn.Linear(v_size, input_size)
        self.dropout = nn.Dropout(drop_rate)
    ...
    def forward(self, input_data):
        ...
        if input_data.ndim == 3:
            output = torch.zeros([input_data.size()[0], input_data.size()[1], int(self.v_size / self.h)]).to(device)
        else:
            output = torch.zeros([input_data.size()[0], int(self.v_size / self.h)]).to(device)
        for i in range(self.h):
